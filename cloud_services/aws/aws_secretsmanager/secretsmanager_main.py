...
class main():
    def __init__(self, awsregion):
        ...
        self.service_name = 'secretsmanager' #service name
    ...
    def get_secret_value(self):
        required_parameters = ["secret_id"]
        missing_parameters = common_utils.check_missing_parms(self.service_object, required_parameters)
        if missing_parameters["status"]:
            get_secret_value_response = common_utils.missing_key_response(missing_parameters)
        else:
            if self.client_status:
                try:
                    response = self.service_client.get_secret_value(
                        SecretId=self.service_object["secret_id"]
                    )
                    if response['ResponseMetadata']['HTTPStatusCode'] != 200:
                        get_secret_value_response = common_utils.error_exception_message(self.service_object,
                                                                                     response)
                    else:
                    ...
                    get_secret_value_response = common_utils.error_exception_message(self.service_object, exception_details, inspect.currentframe().f_back.f_lineno)
            else:
                get_secret_value_response = common_utils.error_exception_message(self.service_object,self.client_exception_details)
        return get_secret_value_response

    def update_secret_value(self):
        required_parameters = ["secret_id","secret_string"]
        missing_parameters = common_utils.check_missing_parms(self.service_object, required_parameters)
        if missing_parameters["status"]:
            update_secret_value_response = common_utils.missing_key_response(missing_parameters)
        else:
            if self.client_status:
                try:
                    response = self.service_client.update_secret(
                        SecretId=self.service_object["secret_id"],
                        SecretString=self.service_object["secret_string"]
                    )
                    if response['ResponseMetadata']['HTTPStatusCode'] != 200:
                        update_secret_value_response = common_utils.error_exception_message(self.service_object,
                                                                                     response)
                    else:
                        update_secret_value_response = {
                            "status": True
                        }
                    self.close_connections()
                except Exception as exception_details:
                    update_secret_value_response = common_utils.error_exception_message(self.service_object, exception_details, inspect.currentframe().f_back.f_lineno)
            else:
                update_secret_value_response = common_utils.error_exception_message(self.service_object,self.client_exception_details)
...
def service_api(service_object):
    ...
    if 'region' in service_object:
        ...
        if 'action' in service_object:
            ...
            if service_action == 'get_secret_value':
                object_response = service_class.get_secret_value()
                response["status"] = object_response["status"]
                response[service_action_response] = object_response
            elif service_action == 'update_secret_value':
                object_response = service_class.update_secret_value()
                response["status"] = object_response["status"]
                response[service_action_response] = object_response
